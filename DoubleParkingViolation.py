...
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "capstone-t5-4494681f8d0c.json"
...
json_path = "capstone-t5-4494681f8d0c.json"
                ...
                if point_inside_polygon(center_x, center_y, points):
                    ...
                    if duration >= violation_threshold_seconds and not objects[track_id]['violated']:
                        ...
                        if plate_text: #then if there is plate insert 
                            cropped_screenshot_path = os.path.join(save_dir, f"violated_{track_id}_cropped_enhanced.jpg")
                            cv2.imwrite(cropped_screenshot_path, enhanced_cropped)
                            print(f"Saved new violated object: {track_id}")
                        # Add to violations database (time , pic , plate in english)
                            try:
                                current_time = datetime.now()
                                with open(cropped_screenshot_path, 'rb') as image_file:
